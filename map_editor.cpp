...
void MapEditor::mouse(const SDL_Event &event, bool left_mouse, bool right_mouse, Coordinate mouse_position)
{
    ...
    uint16_t brush = left_mouse ? left_brush : right_brush;
    last_brush = brush;
    ...
    {
        uint16_t terrain = game->get_base_terrain(position);
        ...
        do
        {
            ...
            promote = (neighbor_mask == 0b00111111) && (multishape.promoted >= 0);
            assert((promote && demote) == false);
            if (promote)
            {
        ...
        uint16_t updated_terrain = game->get_multishape_terrain(group, neighbor_mask);
